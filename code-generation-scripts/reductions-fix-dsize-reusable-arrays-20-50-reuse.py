...
def gen_program(n_stmts, n_arrs):
    stmts = [
        Mapping(
            '_',
            [f'scalar{i} = scalar{i} + #_:init# + #_:init# + #_:init# + #_:init# + #_# + #_# + #_# + #_# + #_# + #_#;' for i in range(1, n_stmts + 1)] +
            [';'] * (5 - n_stmts),
            is_finite = True,
        )
    ]
    init_exprs = [
        Mapping(
            'init',
            [f'Arr{i:02d}[i]' for i in range(1, 21)],
            is_finite = True
        )
    ]
    zeros = [
    ...
    program = program.fill_expressions(init_exprs)
    program = program.fill_expressions(zeros)
    program = program.fill_expressions(rest_exprs)
    return program

seed(0)
application = 'LoopGen'
batch = 'reduction-fix-dsize-reusable-arrays-20-50-reuse'

programs = set()
source_infos = []
for n_stmts in range(5, 6):
    for n_arrs in range(20, 51):
        skeleton = gen_program(n_stmts, n_arrs)
        code = f'{n_stmts}_scalars_{n_arrs:02d}_arrs'
        codelet = f'{code}_de'
        ...
        config.output_dir = generate_codelet_files(application, batch, code, codelet, 10, [32000, 20])
