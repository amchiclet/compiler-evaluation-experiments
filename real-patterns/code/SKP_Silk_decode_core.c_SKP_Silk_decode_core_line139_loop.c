...
extern int inv_gain_Q32;
extern short  sLTP[20 * 24] ;
...
#pragma scop
    ...
    for(i = 0; i < lag + 5 / 2; i++)
    {
        psDec -> sLTP_Q16[sLTP_buf_idx - i - 1] =(inv_gain_Q32 >> 16) *((int )((short )sLTP[psDec -> frame_length - i - 1])) +((inv_gain_Q32 & 0x0000FFFF) *((int )((short )sLTP[psDec -> frame_length - i - 1])) >> 16);
    }
