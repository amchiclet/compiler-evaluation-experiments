...
extern StorablePicture * restrict fs;
extern int jj;
extern int i;
extern int ii;
struct colocated_params
{
    ...
    int64 top_ref_pic_num[6][33];
    char ***top_ref_idx;
    int64 ***top_ref_pic_id;
    short ****top_mv;
    byte **top_moving_block;
    int64 bottom_ref_pic_num[6][33];
    char ***bottom_ref_idx;
    int64 ***bottom_ref_pic_id;
    short ****bottom_mv;
...
extern ColocatedParams * restrict p;
...
#pragma scop
    ...
    for(j = 0; j < fs -> size_y / 4; j++)
    {
        ...
        for(i = 0; i < fs -> size_x / 4; i++)
        {
            ...
            {
                p -> moving_block[j][i] =(!(!p -> is_long_term &&(p -> ref_idx[LIST_0][j][i] == 0 &&((p -> mv[LIST_0][j][i][0] < 0?-p -> mv[LIST_0][j][i][0] : p -> mv[LIST_0][j][i][0])) >> 1 == 0 &&((p -> mv[LIST_0][j][i][1] < 0?-p -> mv[LIST_0][j][i][1] : p -> mv[LIST_0][j][i][1])) >> 1 == 0) || p -> ref_idx[LIST_0][j][i] == - 1 && p -> ref_idx[LIST_1][j][i] == 0 &&((p -> mv[LIST_1][j][i][0] < 0?-p -> mv[LIST_1][j][i][0] : p -> mv[LIST_1][j][i][0])) >> 1 == 0 &&((p -> mv[LIST_1][j][i][1] < 0?-p -> mv[LIST_1][j][i][1] : p -> mv[LIST_1][j][i][1])) >> 1 == 0));
            }
        }
    }
