...
#pragma scop
    ...
    for(j = 0; j < fs -> size_y / 4; j++)
    {
        ...
        for(i = 0; i < fs -> size_x / 4; i++)
        {
            ...
            {
                p -> moving_block[j][i] =(!(!p -> is_long_term &&(p -> ref_idx[LIST_0][j][i] == 0 && abs(p -> mv[LIST_0][j][i][0]) >> 1 == 0 && abs(p -> mv[LIST_0][j][i][1]) >> 1 == 0) || p -> ref_idx[LIST_0][j][i] == - 1 && p -> ref_idx[LIST_1][j][i] == 0 && abs(p -> mv[LIST_1][j][i][0]) >> 1 == 0 && abs(p -> mv[LIST_1][j][i][1]) >> 1 == 0));
            }
        }
    }
