...
#pragma scop
    ...
    for(j = 0; j < cr_MB_y; j++) for(i = 0; i < cr_MB_x; i++) img -> mpr[i][j] =((0 >((img -> max_imgpel_value_uv < iaa +(i - cr_MB_x / 2 + 1) * ib +(j - cr_MB_y / 2 + 1) * ic + 16 >> 5?img -> max_imgpel_value_uv : iaa +(i - cr_MB_x / 2 + 1) * ib +(j - cr_MB_y / 2 + 1) * ic + 16 >> 5))?0 :((img -> max_imgpel_value_uv < iaa +(i - cr_MB_x / 2 + 1) * ib +(j - cr_MB_y / 2 + 1) * ic + 16 >> 5?img -> max_imgpel_value_uv : iaa +(i - cr_MB_x / 2 + 1) * ib +(j - cr_MB_y / 2 + 1) * ic + 16 >> 5))));
