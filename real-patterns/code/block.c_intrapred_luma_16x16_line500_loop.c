...
#pragma scop
    ...
    for(j = 0; j < 16; j++)
    {
        for(i = 0; i < 16; i++)
        {
            img -> mprr_2[3][j][i] =((0 >((((int )(img -> max_imgpel_value)) <(iaa +(i - 7) * ib +(j - 7) * ic + 16) / 32?((int )(img -> max_imgpel_value)) :(iaa +(i - 7) * ib +(j - 7) * ic + 16) / 32))?0 :((((int )(img -> max_imgpel_value)) <(iaa +(i - 7) * ib +(j - 7) * ic + 16) / 32?((int )(img -> max_imgpel_value)) :(iaa +(i - 7) * ib +(j - 7) * ic + 16) / 32))));
        }
    }
