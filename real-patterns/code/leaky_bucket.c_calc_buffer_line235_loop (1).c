...
#pragma scop
    ...
    {
        if(iBucket == 0) Rmin[iBucket] =((unsigned long )(((float )AvgRate) * img -> framerate)) /(input -> jumpd + 1);
        else Rmin[iBucket] =((unsigned long )(((float )Rmin[iBucket - 1]) +(AvgRate / 4) * img -> framerate /(input -> jumpd + 1)));
    }
