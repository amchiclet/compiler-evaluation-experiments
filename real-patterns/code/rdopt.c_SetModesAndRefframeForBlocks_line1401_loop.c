...
extern StorablePicture * restrict enc_picture;
enum LIST_0_enum {LIST_0=0,LIST_1=1,BI_PRED=2,BI_PRED_L0=3,BI_PRED_L1=4};;
extern char *** restrict direct_ref_idx;
extern Macroblock * restrict currMB;
extern short  best8x8pdir[15][4] ;
extern int bframe;
extern short  best8x8fwref[15][4] ;
extern short  best8x8bwref[15][4] ;
...
#pragma scop
    ...
    for(j = 0; j < 4; j++)
    {
        ...
        for(i = 0; i < 4; i++)
        {
            ...
            else if(mode == 1 && currMB -> bi_pred_me &&((best8x8pdir[mode][k] == 0 || best8x8pdir[mode][k] == 2) &&(mode != 8 || best8x8mode[k] != 0 || !bframe)) &&((best8x8pdir[mode][k] == 1 || best8x8pdir[mode][k] == 2) &&(mode != 8 || best8x8mode[k] != 0)))
            {
            ...
            {
                enc_picture -> ref_idx[LIST_0][block_y][block_x] =(((best8x8pdir[mode][k] == 0 || best8x8pdir[mode][k] == 2) &&(mode != 8 || best8x8mode[k] != 0 || !bframe)?best8x8fwref[mode][k] : - 1));
                enc_picture -> ref_idx[LIST_1][block_y][block_x] =(((best8x8pdir[mode][k] == 1 || best8x8pdir[mode][k] == 2) &&(mode != 8 || best8x8mode[k] != 0)?best8x8bwref[mode][k] : - 1));
            }
        }
    }
