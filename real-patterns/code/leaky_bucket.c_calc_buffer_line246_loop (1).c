...
#include <stdio.h>
#include <stdlib.h>
extern unsigned long iBucket;
extern unsigned long NumberLeakyBuckets;
extern long iChannelRate;
extern unsigned long * restrict Rmin;
...
typedef struct
{
    ...
    char sp_output_filename[200];
    char sp2_input_filename1[200];
    char sp2_input_filename2[200];
    ...
    int AdaptRndPeriod;
    int AdaptRndChroma;
    int AdaptRndWFactor[2][5];
    int EarlySkipEnable;
    int SelectiveIntraEnable;
    int DisposableP;
    int DispPQPOffset;
} InputParameters;
extern InputParameters * restrict input;
typedef struct
{
    int byte_pos;
    int bits_to_go;
...
extern long InitFullness;
extern long maxBuffer;
extern long * restrict buffer_frame;
extern long minB;
extern unsigned long iFrame;
extern unsigned long total_frame_buffer;
extern long  Bit_Buffer[10000] ;
extern unsigned long FrameIndex;
extern long actualBuffer;
extern unsigned long * restrict Bmin;
extern unsigned long * restrict Fmin;
...
#pragma scop
    ...
    {
        iChannelRate =((long )((Rmin[iBucket] *(input -> jumpd + 1)) / img -> framerate));
        InitFullness = maxBuffer;
        ...
        for(iFrame = 0; iFrame < total_frame_buffer; iFrame++)
        {
            ...
            if(buffer_frame[iFrame + 1] > maxBuffer) buffer_frame[iFrame + 1] = maxBuffer;
        }
        ...
        for(iFrame = 0; iFrame < FrameIndex + 1; iFrame++)
        {
            ...
            buffer_frame[iFrame + 1] = buffer_frame[iFrame] + iChannelRate;
            if(buffer_frame[iFrame + 1] > actualBuffer) break;
        }
