...
#pragma scop
    ...
    for(j = img -> block_y; j < img -> block_y + 4; j++) for(i = img -> block_x; i < img -> block_x + 4; i++)
        {
            cur_ref[LIST_0] =((int )enc_picture -> ref_idx[LIST_0][j][i]);
            cur_ref[LIST_1] =((int )enc_picture -> ref_idx[LIST_1][j][i]);
            enc_picture -> ref_pic_id[LIST_0][j][i] =(cur_ref[LIST_0] >= 0?enc_picture -> ref_pic_num[LIST_0 + currMB -> list_offset][cur_ref[LIST_0]] :(- 1));
            enc_picture -> ref_pic_id[LIST_1][j][i] =(cur_ref[LIST_1] >= 0?enc_picture -> ref_pic_num[LIST_1 + currMB -> list_offset][cur_ref[LIST_1]] :(- 1));
        }
