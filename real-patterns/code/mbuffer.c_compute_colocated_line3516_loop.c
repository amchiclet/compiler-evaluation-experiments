...
extern StorablePicture * restrict fs_bottom;
extern StorablePicture * restrict fs_top;
...
#pragma scop
    ...
    for(j = 0; j < fs -> size_y / 4; j++)
    {
        ...
        for(i = 0; i < fs -> size_x / 4; i++)
        {
            ...
            {
                if(((enc_picture -> poc - fs_bottom -> poc < 0?-(enc_picture -> poc - fs_bottom -> poc) : enc_picture -> poc - fs_bottom -> poc)) >((enc_picture -> poc - fs_top -> poc < 0?-(enc_picture -> poc - fs_top -> poc) : enc_picture -> poc - fs_top -> poc)))
                {
