...
extern short pix_c_y;
extern int i;
extern short pix_c_x;
...
#pragma scop
    ...
    for(j = 0; j < img -> mb_cr_size_y; j++)
    {
        ...
        for(i = 0; i < img -> mb_cr_size_x; i++)
        {
            ...
            if(lossless_qpprime) enc_picture -> imgUV[uv][pix_c_y][pix_c_x] =(img -> m7[j][i] + img -> mpr[j][i]);
            else enc_picture -> imgUV[uv][pix_c_y][pix_c_x] = img -> m7[j][i];
        }
    }
