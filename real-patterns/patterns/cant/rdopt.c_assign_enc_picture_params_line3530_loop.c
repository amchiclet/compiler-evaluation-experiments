...
extern int best_bw_ref;
extern short * restrict cur_mv;
typedef struct
{
    int byte_pos;
    int bits_to_go;
...
#pragma scop
    ...
    for(i = 0; i < 16 / 4; i++)
    {
        enc_picture -> ref_pic_id[LIST_1][block_y][block_x + i] = enc_picture -> ref_pic_num[LIST_1 + list_offset][best_bw_ref];
        if(best_bw_ref >= 0)
        {
