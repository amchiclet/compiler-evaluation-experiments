...
def report_for_compiler_group(compiler_names, table):
    ...
    baseline = {}
    for executable_name, _ in executables:
        program_name, variant_name = executable_name.split('_')[-3:-1]
        row = [executable_name]
        for c in compiler_names:
            if c not in baseline:
                baseline[c] = {}
            if executable_name not in baseline[c]:
                baseline[c][executable_name] = None
            compiler_s = c + '_s'
            runtime_s = None
            runtime_range_s = 'N/A'
            normalized_runtime_s = 'N/A'
            if compiler_s in table and \
               executable_name in table[compiler_s]['runtime']:
                runtime_range_s = table[compiler_s]['runtime'][executable_name]
                runtime_s = float(runtime_range_s.split()[0])
                if variant_name == 'v00':
                    baseline[c][program_name] = runtime_s
            base = baseline[c][program_name]

            if base is not None and runtime_s is not None:
                normalized_runtime_s = str(base / runtime_s)

            compiler_v = c + '_v'
            runtime_v = None
            runtime_range_v = 'N/A'
            normalized_runtime_v = 'N/A'
            speed_up = 'N/A'
            if compiler_v in table and \
               executable_name in table[compiler_v]['runtime']:
                runtime_range_v = table[compiler_v]['runtime'][executable_name]
                runtime_v = float(runtime_range_v.split()[0])
                if runtime_s is not None:
                    speed_up = str(runtime_s / runtime_v)

            if base is not None and runtime_v is not None:
                normalized_runtime_v = str(base / runtime_v)

            if variant_name == 'v00':
                row.append(runtime_range_s)
                row.append(runtime_range_v)
            else:
                row.append(normalized_runtime_s)
                row.append(normalized_runtime_v)
            row.append(speed_up)
