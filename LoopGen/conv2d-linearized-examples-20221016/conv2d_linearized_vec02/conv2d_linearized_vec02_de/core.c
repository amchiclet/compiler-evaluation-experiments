...
int weight_ofm_stride;
  ...
  input_ifm_stride = (IFHP * IFWP) * 2;
  output_img_stride = (OFM * OFH) * OFW;
  output_ofm_stride = (OFH * OFW) * 2;
  weight_ofm_stride = ((IFM * KH) * KW) * 2;
  weight_ifm_stride = ((KH * KW) * 2) * 2;
  for (int img = 0; img <= IMG - 1; img += 1) {
    for (int ifm_tile = 0; ifm_tile <= IFM / 2 - 1; ifm_tile += 1) {
      for (int ofm_tile = 0; ofm_tile <= OFM / 2 - 1; ofm_tile += 1) {
        for (int ofm = 0; ofm <= 2 - 1; ofm += 1) {
            ...
            for (int oi = 0; oi <= OFH - 1; oi += 1) {
              ...
              for (int kj = kj_begin; kj <= kj_end - 1; kj += 1) {
                for (int ki = ki_begin; ki <= ki_end - 1; ki += 1) {
                  for (int ifm = 0; ifm <= 2 - 1; ifm += 1) {
                    (*input)[(((img * input_img_stride + ifm_tile * input_ifm_stride) + (((ij + kj) + PH) * IFWP) * 2) + ((ii + ki) + PW) * 2) + ifm] = (*input)[(((img * input_img_stride + ifm_tile * input_ifm_stride) + (((ij + kj) + PH) * IFWP) * 2) + ((ii + ki) + PW) * 2) + ifm] + (*output)[(((img * output_img_stride + ofm_tile * output_ofm_stride) + (oj * OFW) * 2) + oi * 2) + ofm] * (*filter)[((((ofm_tile * weight_ofm_stride + ifm_tile * weight_ifm_stride) + ((kj * KW) * 2) * 2) + (ki * 2) * 2) + ofm * 2) + ifm];
                  }
                }
              }
            }
